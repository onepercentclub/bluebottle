...
class ActivityListSearchAPITestCase(ESTestCase, BluebottleTestCase):
    ...
    def search(self, filter, sort=None, user=None, place=None, headers=None):
        ...
        response = self.client.get(url, user=user, **headers)
    ...
    def assertFacets(self, filter, facets):
        counts = dict(
            ...
            for facet in self.data["meta"]["facets"][filter]
        )
        names = dict(
            (facet["id"], facet.get("name"))
            for facet in self.data["meta"]["facets"][filter]
        )
    ...
    def test_images(self):
        ...
        for activity in response.json()["data"]:
            self.assertTrue(
                re.match(
                    "^/api/activities/\d+/image/600x337",
                    activity["attributes"]["image"],
                )
            )
    ...
    def test_date_preview(self):
        ...
        self.assertEqual(
            dateutil.parser.parse(attributes["start"]), activity.slots.first().start
        )
        self.assertEqual(
            dateutil.parser.parse(attributes["end"]), activity.slots.first().end
        )
        self.assertEqual(attributes["has-multiple-locations"], False)
        location = activity.slots.first().location
        self.assertEqual(
            attributes["location"],
            f"{location.locality}, {location.country.alpha2_code}",
        )
    ...
    def test_date_preview_multiple_slots_single_open(self):
        activity = DateActivityFactory.create(status="open", slots=[])
        DateActivitySlotFactory.create(
            activity=activity, status="draft", is_online=None
        )
    ...
    def test_date_preview_multiple_slots_filtered(self):
        ...
        activity = DateActivityFactory.create(status="open", slots=[])
        DateActivitySlotFactory.create(
            activity=activity, start=now() + timedelta(days=14)
        )
        DateActivitySlotFactory.create(
            activity=activity, start=now() + timedelta(days=21)
        )
        current_slot = DateActivitySlotFactory.create(
            activity=activity, start=now() + timedelta(days=7)
        )
    ...
    def test_date_preview_multiple_slots_succeeded(self):
        ...
        last = DateActivitySlotFactory.create(
            activity=activity, start=now() - timedelta(days=7)
        )
    ...
    def test_deadline_preview(self):
        ...
        self.assertEqual(attributes["has-multiple-locations"], False)
        self.assertEqual(
            attributes["contribution-duration"], {"period": "once", "value": 4.0}
        )
    ...
    def test_search(self):
        ...
        DateActivitySlotFactory.create(
            activity=slot_title, title=f"slot title with {text}"
        )
    ...
    def test_sort_distance(self):
        ...
        # With online
        self.search(
            filter={"distance": "500km"}, sort="distance", place=leiden_place.pk
        )
        data = self.data["data"]
    ...
    def test_sort_date(self):
        ...
        DateActivitySlotFactory.create(
            status="open", start=now() + timedelta(days=2), activity=activities[1]
        )
        DateActivitySlotFactory.create(
            status="open", start=now() + timedelta(days=5), activity=activities[1]
        )

        DateActivitySlotFactory.create(
            status="open", start=now() + timedelta(days=4), activity=activities[2]
        )
        DateActivitySlotFactory.create(
            status="open", start=now() + timedelta(days=7), activity=activities[2]
        )
    ...
    def test_filter_segment(self):
        segment_type = SegmentTypeFactory.create(is_active=True, enable_search=True)
        matching_segment, other_segment = SegmentFactory.create_batch(
            2, segment_type=segment_type
        )
    ...
    def test_filter_theme_dutch(self):
        ...
        self.assertFacets(
            "theme",
            {
                str(matching_initiative.theme.pk): (
                    matching_theme_translation.name,
                    len(matching),
                ),
                str(other_initiative.theme.pk): (
                    other_theme_translation.name,
                    len(other),
                ),
            },
        )
    ...
    def test_filter_initiative(self):
        ...
        draft = DeedFactory.create(
            status="draft", initiative=initiative, owner=draft_owner
        )
    ...
    def test_filter_team(self):
        InitiativePlatformSettings.objects.create(
            activity_search_filters=["team_activity"]
        )

        matching = DeadlineActivityFactory.create_batch(
            2, status="open", team_activity="teams"
        )
        other = DeadlineActivityFactory.create_batch(
            3, status="open", team_activity="individuals"
        )
        ...
        self.assertFacets(
            "team_activity",
            {
                "teams": ("With your team", len(matching)),
                "individuals": ("As an individual", len(other)),
            },
        )
        self.assertFound(matching)
    ...
    def test_filter_team_no_matching(self):
        ...
        self.search(
            {"team_activity": "teams", "country": "something-that-does-not-match"}
        )
    ...
    def test_more_country_facets(self):
        ...
        for country in countries:
            ...
            matching.append(
                DeadlineActivityFactory.create(location=location, status="open")
            )
    ...
    def test_filter_highlight(self):
        ...
        self.assertFacets(
            "highlight", {1: ("Yes", len(matching)), 0: ("No", len(other))}
        )
        self.assertFound(matching)
    ...
    def test_filter_date(self):
        matching = [
            DeadlineActivityFactory.create(
                status="open", start="2025-04-01", deadline="2025-04-02"
            ),
            DeadlineActivityFactory.create(
                status="open", start="2025-04-01", deadline="2025-04-03"
            ),
            DeedFactory.create(status="open", start="2025-04-05", end="2025-04-07"),
            CollectActivityFactory.create(
                status="open", start="2025-04-05", end="2025-04-07"
            ),
        ]

        DeadlineActivityFactory.create(
            status="open", start="2025-05-01", deadline="2025-05-02"
        )
        DeadlineActivityFactory.create(
            status="open", start="2025-05-01", deadline="2025-05-03"
        )
        DeedFactory.create(status="open", start="2025-05-05", end="2025-05-07")
        CollectActivityFactory.create(
            status="open", start="2025-05-05", end="2025-05-07"
        )
    ...
    def test_filter_past_dates(self):
        ...
        activity4 = DeadlineActivityFactory.create(
            status="succeeded", start="2022-04-15", deadline="2022-05-15"
        )
        DeadlineActivityFactory.create(
            status="succeeded", start="2022-03-01", deadline="2022-06-01"
        )

        DateActivitySlotFactory.create(
            activity=activity1, start=datetime.datetime(2022, 5, 3, tzinfo=UTC)
        )
        DateActivitySlotFactory.create(
            activity=activity1, start=datetime.datetime(2022, 5, 25, tzinfo=UTC)
        )
        DateActivitySlotFactory.create(
            activity=activity1, start=datetime.datetime(2022, 6, 3, tzinfo=UTC)
        )

        DateActivitySlotFactory.create(
            activity=activity2, start=datetime.datetime(2022, 5, 30, tzinfo=UTC)
        )
        DateActivitySlotFactory.create(
            activity=activity2, start=datetime.datetime(2022, 5, 25, tzinfo=UTC)
        )
        DateActivitySlotFactory.create(
            activity=activity2, start=datetime.datetime(2022, 4, 25, tzinfo=UTC)
        )

        DateActivitySlotFactory.create(
            activity=activity3, start=datetime.datetime(2022, 6, 3, tzinfo=UTC)
        )
        DateActivitySlotFactory.create(
            activity=activity3, start=datetime.datetime(2022, 4, 23, tzinfo=UTC)
        )
        ...
        self.assertFound(matching)
        self.assertEqual(
            self.data["data"][0]["attributes"]["end"], "2022-05-30T02:00:00+00:00"
        )
        self.assertEqual(
            self.data["data"][1]["attributes"]["end"], "2022-05-25T02:00:00+00:00"
        )
    ...
    def test_filter_distance(self):
        ...
        matching = [
            ...
            DeadlineActivityFactory.create(
                status="open",
                location=GeolocationFactory.create(
                    position=Point(lon + 0.1, lat + 0.1)
                ),
            ),
            DeadlineActivityFactory.create(
                status="open",
    ...
    def test_filter_distance_with_online(self):
        ...
        DateActivitySlotFactory.create(
            activity=matching[0], location=GeolocationFactory.create(position=leiden)
        )
        DateActivitySlotFactory.create(
            activity=matching[1], location=GeolocationFactory.create(position=amsterdam)
        )
        ...
        DateActivitySlotFactory.create(
            activity=other, location=GeolocationFactory.create(position=lyutidol)
        )
    ...
    def test_filter_office_restriction(self):
        ...
        matching = [
            ...
            DeadlineActivityFactory.create(
                status="open",
                office_location=within_region,
                office_restriction="office_region",
            ),
            DeadlineActivityFactory.create(
                status="open",
                office_location=within_sub_region,
    ...
    def test_create(self):
        data = {
            "data": {
                ...
                "relationships": {
                    "image": {
                        "data": {"type": "images", "id": self.file_data["data"]["id"]}
                    },
                    "resource": {
                        "data": {
        ...
        response = self.client.post(
            self.related_image_url, data=json.dumps(data), user=self.owner
        )

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        self.assertEqual(
            response.json()["included"][1]["attributes"]["links"]["large"].split("?")[
                0
            ],
            "/api/activities/{}/related-image/600".format(
...
class ContributorListAPITestCase(BluebottleTestCase):
    ...
    def test_get(self):
        ...
        for contributor in data["data"]:
            ...
            self.assertTrue(
                contributor["relationships"]["activity"]["data"]["type"]
                in (
                    "activities/fundings",
                    "activities/deeds",
            ...
            if contributor["type"] == "contributors/time-based/date-participants":
                self.assertEqual(
                    contributor["attributes"]["total-duration"], "02:00:00"
                )
...
class ActivityAPIAnonymizationTestCase(ESTestCase, BluebottleTestCase):
    ...
    def test_activity_over_max_age(self):
        ...
        activity = DateActivityFactory.create(
            created=now() - timedelta(days=400), status="open"
        )
    ...
    def test_initiative_not_over_max_age(self):
        ...
        self.assertEqual(
            data["data"]["relationships"]["reviewer"]["data"]["id"],
            str(initiative.reviewer.pk),
        )
    ...
    def test_participants_over_max_age(self):
        ...
        new_participant = DateParticipantFactory.create(activity=activity)
        ...
        self.assertEqual(
            data["data"][1]["relationships"]["user"]["data"]["id"], "anonymous"
        )
        self.assertEqual(
