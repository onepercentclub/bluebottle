...
class FederatedObjectBaseSerializer(
    ...
    def create(self, validated_data):
        ...
        if origin and hasattr(origin, 'adopted') and origin.adopted_id != result.pk:
            from bluebottle.activity_pub.models import Image as ActivityPubImage

            if not (
                isinstance(origin, ActivityPubImage) and
                ActivityPubImage.objects.filter(adopted_id=result.pk).exists()
            ):
