...
from bluebottle.time_based.models import (
    DateActivitySlot, DeadlineActivity, DateActivity,
    PeriodicSlot, RegisteredDateActivity,
    PeriodicActivity, ScheduleActivity, ScheduleSlot,
    TeamScheduleSlot,
    Team
)
...
class RelatedCaptainField(RelatedField):
    def get_queryset(self):
        # TODO: filter queryset on correct types
        return DateActivity.objects.all()

    def to_representation(self, value):
        user = value.user if value.user else value.remote_user

        if hasattr(user, 'activity_pub_model'):
            return user.activity_pub_model.pub_url
        else:
            return adapter.sync(user).pub_url
    ...
    def to_internal_value(self, data):
        ...
        if activity_pub_model.is_local:
            return {'user': activity_pub_model.origin}
        else:
            return {'remote_user': activity_pub_model.adopted}


class TeamSerializer(FederatedObjectBaseSerializer):
    type = TypeField('Team')
    name = serializers.CharField(allow_null=True)
    summary = serializers.CharField(
        source='description',
    ...
    captain = RelatedCaptainField(source='*')
