...
class TestCreateDonation(OrderApiTestCase):
    ...
    def test_create_donation(self):
        ...
        response = self.client.post(self.manage_donation_list_url, donation1, HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        donation1['amount'] = 50
        response = self.client.put(donation_url, json.dumps(donation1), 'application/json', HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.post(self.manage_donation_list_url, donation2, HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['status'], 'new')
        ...
        response = self.client.delete(donation_url, 'application/json', HTTP_AUTHORIZATION=self.user_token)
