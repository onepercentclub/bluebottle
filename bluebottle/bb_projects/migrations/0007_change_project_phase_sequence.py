...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Add vote stati."
        orm['bb_projects.ProjectPhase'].objects.filter(sequence=11).update(sequence=4)
        orm['bb_projects.ProjectPhase'].objects.filter(sequence=12).update(sequence=5)

    def backwards(self, orm):
        "Write your backwards methods here."
        orm['bb_projects.ProjectPhase'].objects.filter(sequence=4).update(sequence=11)
        orm['bb_projects.ProjectPhase'].objects.filter(sequence=5).update(sequence=12)
