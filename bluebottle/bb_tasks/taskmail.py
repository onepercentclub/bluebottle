...
TASK_MODEL = get_model_class('TASKS_TASK_MODEL')
TASK_MEMBER_MODEL = get_model_class('TASKS_TASKMEMBER_MODEL')
...
def new_reaction_notification(sender, instance, created, **kwargs):
    ...
    if task_member.status == TASK_MEMBER_MODEL.TaskMemberStatuses.realized:
        ...
        link = '/go/tasks/{0}'.format(task.id)
        task_list = '/go/tasks'
        project_link = '/go/projects/{0}'.format(task.project.slug)
