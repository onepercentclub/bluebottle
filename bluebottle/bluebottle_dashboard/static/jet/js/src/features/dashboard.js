...
Dashboard.prototype = {
  ...
  initModuleChildrenFormsetUpdate: function($dashboard) {
    ...
    $inline.find(".add-row a").on("click", function(e) {
      ...
      self.updateModuleChildrenFormsetFormIndex(
        $empty,
        parseInt($inline.find(".inline-related").length) - 1
      );
      self.updateModuleChildrenFormsetFormIndex(
        $clone,
        parseInt($inline.find(".inline-related").length) - 2
      );
