...
class TestTenantSiteReplacement(TestCase):
    def test_domain(self):
        with mock.patch("bluebottle.clients.utils.connection.tenant",
            ...
            self.assertEqual(tenant_url(), "https://example.com")

    def test_name(self):
        with mock.patch("bluebottle.clients.utils.connection.tenant",
                        FixMock(name="Acme Inc.")):
            self.assertEqual(tenant_name(), "Acme Inc.")

    def test_site(self):
        with mock.patch("bluebottle.clients.utils.connection.tenant",
                        FixMock(domain_url="acme.com", name="Acme Inc.")):
            site = tenant_site()
            self.assertEqual(site.domain, "acme.com")
            self.assertEqual(site.name, "Acme Inc.")
