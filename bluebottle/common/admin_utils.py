...
class VerboseForeignKeyRawIdWidget(ForeignKeyRawIdWidget):
    def label_for_value(self, value):
        ...
        try:
            ...
            change_url = reverse(
                "admin:{0}_{1}_change".format(obj._meta.app_label, obj._meta.object_name.lower()),
                args=(obj.pk,)
            )
            return '&nbsp;<strong><a href="{0}">{1}</a></strong>'.format(change_url, escape(obj))
    ...
    def label_for_value(self, value):
        ...
        for v in values:
            try:
                ...
                str_values += ['<strong><a href="{0}">{1}</a></strong>'.format(change_url, escape(x))]
