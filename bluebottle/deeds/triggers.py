...
from bluebottle.deeds.effects import (
    ...
    triggers = ActivityTriggers.triggers + [
        ModelChangedTrigger(
            ...
            effects=[
                ...
                TransitionEffect(
                    DeedStateMachine.succeed, conditions=[is_finished, has_participants]
                ),
                TransitionEffect(
                    DeedStateMachine.expire,
                    conditions=[is_finished, has_no_participants],
                ),
                RescheduleEffortsEffect,
                NotificationEffect(
                    DeedDateChangedNotification, conditions=[is_not_finished]
                ),
            ],
        ),
        ModelChangedTrigger(
            ...
            effects=[
                ...
                RescheduleEffortsEffect,
                NotificationEffect(
                    DeedDateChangedNotification, conditions=[is_not_started]
                ),
            ],
        ),
        ModelChangedTrigger(
            "enable_impact", effects=[UpdateImpactGoalsForActivityEffect]
        ),
...
def activity_did_start(effect):
    ...
    return (
        not effect.instance.activity.start
        or effect.instance.activity.start < date.today()
    )
...
class DeedParticipantTriggers(ContributorTriggers):
    triggers = ContributorTriggers.triggers + [
        TransitionTrigger(
            ...
            effects=[
                TransitionEffect(
                    DeedParticipantStateMachine.succeed, conditions=[activity_did_start]
                ),
                CreateEffortContribution,
                NotificationEffect(NewParticipantNotification, conditions=[is_user]),
                NotificationEffect(
                    ParticipantAddedNotification, conditions=[is_not_user]
                ),
                NotificationEffect(
                    ManagerParticipantAddedOwnerNotification,
        ...
        TransitionTrigger(
            ...
            effects=[
                ...
                RelatedTransitionEffect(
                    "contributions", EffortContributionStateMachine.fail
                ),
                NotificationEffect(
                    ParticipantRemovedOwnerNotification, conditions=[is_not_owner]
                ),
                NotificationEffect(
                    ParticipantRemovedNotification, conditions=[is_not_owner]
                ),
        ...
        TransitionTrigger(
            ...
            effects=[
                RelatedTransitionEffect(
                    "contributions", EffortContributionStateMachine.reset
                ),
                FollowActivityEffect,
            ],
        ),
