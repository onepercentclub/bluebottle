...
class TestEventAdmin(BluebottleAdminTestCase):
    ...
    def test_delete_participant_event_admin(self):
        ...
        data = {
            ...
            'intentions-0-intention_ptr': self.participants[0].intention_ptr_id,
            'intentions-0-activity': self.event.id,
            'intentions-0-user': self.participants[0].user_id,
            'intentions-0-time_spent': self.participants[0].time_spent,
            'intentions-0-DELETE': 'on',
            'intentions-1-intention_ptr': self.participants[1].intention_ptr_id,
            'intentions-1-activity': self.event.id,
            'intentions-1-user': self.participants[1].user_id,
            'intentions-1-time_spent': self.participants[1].time_spent,
            'intentions-2-intention_ptr': self.participants[2].intention_ptr_id,
            'intentions-2-activity': self.event.id,
            'intentions-2-user': self.participants[2].user_id,
            'intentions-2-time_spent': self.participants[2].time_spent,
