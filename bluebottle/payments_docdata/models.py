...
class AbstractDocdataPayment(Payment):
    ...
    # Track received information
    # Additional fields from the existing Docdata data. This data comes from
    # the existing DocDataPaymentOrder model that is migrated.
...
class DocdataTransaction(Transaction):
    ...
    The base model for a docdata payment. The model can be used for a
    web menu payment.
    """
    # Note: We're not using DjangoChoices here so that we can write
    # unknown statuses if they are presented by DocData.
