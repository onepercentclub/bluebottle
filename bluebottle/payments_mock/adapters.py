...
class MockPaymentAdapter(BasePaymentAdapter):
    ...
    def create_payment(self):
        """
        Have some basic criteria that might fail so we can check
        our error parsing.
        """
    ...
    def get_authorization_action(self):
        ...
        """
        url = reverse('payment-service-provider',
    ...
    def _get_mapped_status(self, status):
        """
        Helper to map the status of a PSP specific status (Mock PSP) to our
        own status pipeline for an OrderPayment. The status of a MockPayment
        maps 1-1 to OrderStatus so we can return the status
        """
        return status
