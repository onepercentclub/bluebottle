...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        save triggers signal that will make a Journal. This migration imitates
        what happens in a normal save, by calling create_journal_for_sender()
        with an extra parameter to determine that the call comes from the datamigration.

        This is a way to let the function know the related names that it normally gets
        as properties on the Django Model Class, and that will not work since here its done via the orm.Model
        """
        from bluebottle.journals.models import create_journal_for_sender

        for proj_payout in orm.ProjectPayout.objects.all():
            data_dict = {
                'journal_class': orm['journals.ProjectPayoutJournal'],
                'related_model_name': 'payout',
                'related_model_amount_field_name': 'amount_payable',
            }
            create_journal_for_sender(sender=orm.OrganizationPayout, instance=proj_payout, created=False, data_migration=data_dict)

        for org_payout in orm.OrganizationPayout.objects.all():
            data_dict = {
                'journal_class': orm['journals.OrganizationPayoutJournal'],
                'related_model_name': 'payout',
                'related_model_amount_field_name': 'payable_amount_incl',
            }
            create_journal_for_sender(sender=orm.OrganizationPayout, instance=org_payout, created=False, data_migration=data_dict)

    def backwards(self, orm):
        # backwards is tricky, either delete the oldest (initial) journal for
        # all models, but this will also delete a initial journals that were not
        # created with the forwards method.
        pass
    ...
    complete_apps = ['journals', 'payouts']
    symmetrical = True
