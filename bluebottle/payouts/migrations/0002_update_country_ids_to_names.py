...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Replace all foreign keys in the receiver_account_country field with
        country names.
        """
        db.execute(
            """UPDATE payouts_projectpayout
                   SET receiver_account_country = geo_country.name
                   FROM  geo_country
                   WHERE receiver_account_country::int = geo_country.id
                       AND receiver_account_country ~ '^\d+$'"""
        )

    def backwards(self, orm):
        "Do nothing. We always wanted the names instead of the ids"
