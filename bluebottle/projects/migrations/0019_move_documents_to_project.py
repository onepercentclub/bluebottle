...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        path_regex = re.compile('^organizations/')

        for organization_document in orm[
            'organizations.OrganizationDocument'].objects.all():
            # There can be (theoratically) be more then one project connected to the organization
            for project in orm.Project.objects.filter(
                    organization=organization_document.organization):
                # Creare a new document and link it to the project
                file_name = path_regex.sub('projects/',
                                           organization_document.file.name)
                project_document = orm.ProjectDocument(
                    file=file_name,
                    project=project,
                    author=organization_document.author,
                    created=organization_document.created,
                    updated=organization_document.updated,
                    deleted=organization_document.deleted,

                )

                try:
                    # Check if the dir exists
                    target_dir = os.path.dirname(project_document.file.path)
                    ...
                    try:
                        # Move the file
                        os.rename(organization_document.file.path,
                                  project_document.file.path)
                    except OSError:
                        # The file is not there anymore. We probably already moved it
                        pass
                except ValueError:
                    pass
    ...
    complete_apps = ['organizations', 'projects']
    symmetrical = True
