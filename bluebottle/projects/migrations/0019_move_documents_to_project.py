...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for organization_document in orm['organizations.OrganizationDocument'].objects.all():
            # There can be (theoratically) be more then one project connected to the organization
            for project in orm.Project.objects.filter(organization=organization_document.organization):
                # Creare a new document and link it to the project
                file_name = path_regex.sub('projects/', organization_document.file.name)
                ...
                try:
                    ...
                    try:
                        # Move the file
                        os.rename(organization_document.file.path, project_document.file.path)
                    except OSError:
    ...
    models = {
        ...
        u'projects.projectdocument': {
            ...
            'file': ('django.db.models.fields.files.FileField', [], {'max_length': '100'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'documents'", 'to': u"orm['projects.Project']"}),
            'updated': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'blank': 'True'})
        },
