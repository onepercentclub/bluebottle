    ...
    for donor in donor_queryset:
        ...
        for project in top_three_projects:
            ...
            if project.amount_needed - monthly_project.amount <= Money(0, 'EUR'):
                ...
                logger.info(
                    u"New Top3 project added '{0}'".format(new_project.title))
        ...
        if donor.projects.count():
            ...
            recurring_order = create_recurring_order(donor.user,
                                                     ...
                                                     donor)
            logger.debug(
                "Preparing an Order with preferred projects for user: {0}.".format(
                    donor.user))
        else:
            # Prepare MonthlyOrder with Donations for the top three projects.
            logger.debug("Preparing new 'Top Three' Order for user {0}.".format(
                donor.user))
            recurring_order = create_recurring_order(donor.user,
        ...
        logger.info("Starting payment for '{0}' {1}.".format(donor,
                                                             donation_type_message))
