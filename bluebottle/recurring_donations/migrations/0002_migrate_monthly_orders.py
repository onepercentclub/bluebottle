...
class Migration(DataMigration):
    def forwards(self, orm):
        for rec in orm['fund.RecurringDirectDebitPayment'].objects.all():
            donor = orm['recurring_donations.MonthlyDonor'].objects.create(
                user=rec.user,
                amount=Decimal(rec.amount) / 100,
                iban=rec.iban,
                bic=rec.bic,
                name=rec.name,
                city=rec.city,
            ...
            donor.save()
            orders = orm['fund.Order'].objects.filter(user=rec.user,
                                                      recurring=True,
                                                      status='recurring').order_by(
                '-created')
            if orders.count():
                order = orders.all()[0]
                for don in order.donations.all():
                    donor_project = orm[
                        'recurring_donations.MonthlyDonorProject'].objects.create(
                        donor=donor,
                        project=don.project
                    )
                    donor_project.save()
    ...
    def backwards(self, orm):
        ...
        orm['recurring_donations.MonthlyDonor'].objects.all().delete()
        orm['recurring_donations.MonthlyDonorProject'].objects.all().delete()

        orm['recurring_donations.MonthlyBatch'].objects.all().delete()
        orm['recurring_donations.MonthlyOrder'].objects.all().delete()
        orm['recurring_donations.MonthlyDonation'].objects.all().delete()
    ...
    models = {
        ...
        u'recurring_donations.monthlydonation': {
            ...
            'order': ('django.db.models.fields.related.ForeignKey', [],
                      {'related_name': "'donations'",
                       'to': u"orm['recurring_donations.MonthlyOrder']"}),
        ...
        u'recurring_donations.monthlydonorproject': {
            ...
            'donor': ('django.db.models.fields.related.ForeignKey', [],
                      {'related_name': "'projects'",
                       'to': u"orm['recurring_donations.MonthlyDonor']"}),
            u'id': (
            'django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'project': ('django.db.models.fields.related.ForeignKey', [],
                        {'to': u"orm['projects.Project']"})
    ...
    complete_apps = ['fund', 'recurring_donations']
    symmetrical = True
