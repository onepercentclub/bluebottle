...
class Migration(DataMigration):
    def forwards(self, orm):
        """
        Adjust amounts for MonthlyOrder and MonthlyDonation after
        changing it from integer (cents) to decimal (euro).
        """
        for monthly_order in orm[
            'recurring_donations.MonthlyOrder'].objects.all():
            monthly_order.amount /= 100
            monthly_order.save()

        for monthly_donation in orm[
            'recurring_donations.MonthlyDonation'].objects.all():
            monthly_donation.amount /= 100
