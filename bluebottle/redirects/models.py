...
class Redirect(models.Model):
    ...
    regular_expression = models.BooleanField(
        _('Match using regular expressions'), default=False,
        help_text=_("If checked, the redirect-from and redirect-to fields "
    ...
    fallback_redirect = models.BooleanField(
        _("Fallback redirect"), default=False,
        help_text=_("This redirect is only matched after all other redirects "
                    "have failed to match.<br>This allows us to define a "
                    "general 'catch-all' that is only used as a fallback "
                    "after more specific redirects have been attempted."))
