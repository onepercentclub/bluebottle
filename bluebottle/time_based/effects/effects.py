...
class CreateOverallTimeContributionEffect(Effect):
    ...
    def execute(self, **kwargs):
        ...
        if activity.start and activity.start > date.today():
            ...
            contribution_date = tz.localize(
                datetime.combine(
                    activity.deadline, datetime.min.replace(hour=12).time()
                )
            )
        else:
        ...
        if not activity.deadline or start < activity.deadline:
            ...
            TimeContribution.objects.create(
                ...
                start=tz.localize(datetime.combine(start, datetime.min.time())),
                    ...
                    tz.localize(datetime.combine(end, datetime.min.time()))
                    if end
                    else None
                ),
            )
    ...
    def execute(self, **kwargs):
        if self.instance.duration_period == "overall":
            ...
            self.instance.durations.update(
                start=start, end=end, value=self.instance.duration
            )
...
class BaseActiveDurationsTransitionEffect(Effect):
    ...
    def render(cls, effects):
        effect = effects[0]
        users = [
            duration.contributor.user for duration in effect.instance.active_durations
        ]
    ...
    def is_valid(self):
        return super().is_valid and any(
            self.transition in duration.states.possible_transitions()
...
class UnlockUnfilledSlotsEffect(Effect):
    ...
    def slots(self):
        ...
        return [
            ...
            if slot.accepted_participants.count() < slot.capacity
        ]
    ...
    def __repr__(self):
        return "<Effect: Unlock unfilled slots for {activity}>".format(
            activity=self.instance.activity
        )

    def __str__(self):
        return _("Unlock unfilled slots for {activity}").format(
            activity=self.instance.activity
        )
...
class LockFilledSlotsEffect(Effect):
    ...
    def __str__(self):
        return _("Lock filled slots for {activity}").format(
            activity=self.instance.activity
        )
...
class CreateFirstSlotEffect(Effect):
    ...
    def post_save(self):
        tz = get_current_timezone()
        start = (
            tz.localize(datetime.combine(self.instance.start, datetime.min.time()))
            if self.instance.start
            else now()
        )
