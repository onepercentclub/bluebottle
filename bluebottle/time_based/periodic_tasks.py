...
class ActivityFinishedTask(ModelPeriodicTask):
    def get_queryset(self):
        return self.model.objects.filter(
            deadline__lt=date.today(), status__in=["open", "full"]
        )

    effects = [
        TransitionEffect(TimeBasedStateMachine.succeed, conditions=[has_participants]),
        TransitionEffect(
            TimeBasedStateMachine.expire, conditions=[has_no_participants]
        ),
    ]
...
class PeriodicSlotStartedTask(ModelPeriodicTask):
    def get_queryset(self):
        return PeriodicSlot.objects.filter(start__lte=timezone.now(), status="new")

    effects = [TransitionEffect(PeriodicSlotStateMachine.start)]
...
class PeriodicSlotFinishedTask(ModelPeriodicTask):
    def get_queryset(self):
        return PeriodicSlot.objects.filter(end__lte=timezone.now(), status="running")

    effects = [TransitionEffect(PeriodicSlotStateMachine.finish)]
