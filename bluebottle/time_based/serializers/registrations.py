...
class ContactEmailField(serializers.CharField):
    ...
    def to_representation(self, value):
        ...
        if user.is_authenticated and (
            ...
            or user == activity.owner
            or user == activity.initiative.owner
...
class RegistrationSerializer(ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if user not in privileged_users and not user.is_staff and not user.is_superuser:
            del result["answer"]
            del result["document"]
...
class DeadlineRegistrationSerializer(RegistrationSerializer):
    permissions = ResourcePermissionField(
        "deadline-registration-detail", view_args=("pk",)
    )
    participants = ResourceRelatedField(
        many=True, read_only=True, source="deadlineparticipant_set"
    )
...
class ScheduleRegistrationSerializer(RegistrationSerializer):
    permissions = ResourcePermissionField(
        "schedule-registration-detail", view_args=("pk",)
    )
    participants = ResourceRelatedField(
        many=True, read_only=True, source="scheduleparticipant_set"
    )
...
class PeriodicRegistrationSerializer(RegistrationSerializer):
    permissions = ResourcePermissionField(
        "periodic-registration-detail", view_args=("pk",)
    )
    participants = ResourceRelatedField(
        many=True, read_only=True, source="periodicparticipant_set"
    )
