...
from bluebottle.utils.serializers import (
    ...
    AnonymizedResourceRelatedField,
)
    ...
    def get_root_meta(self, resource, many):
        if many:
            try:
                ...
                queryset = (
                    self.context["view"]
                    .queryset.filter(activity_id=int(activity_id))
                    .order_by("start")
                )
...
class DateActivitySlotInfoMixin:
            ...
            if end:
                slots = slots.filter(
                    start__lte=datetime.combine(
                        dateutil.parser.parse(end), time.max
                    ).astimezone(tz)
                )
        except ValueError:
            pass
    ...
    def get_date_info(self, obj):
        ...
        last_slot = (
            obj.slots.exclude(status__in=["draft", "cancelled"])
            .order_by("start")
            .last()
        )
    ...
    def get_location_info(self, obj):
        ...
        has_multiple = (
            len(set(location[:2] for location in locations)) > 1 and not is_online
        )
        if has_multiple:
            return {
                "has_multiple": True,
        ...
        if is_online or not slot.location:
            ...
            location = {
                ...
                "country": {
                    "code": (
                        slot.location.country.alpha2_code
                        if slot.location.country
                        else None
                    ),
                },
                "formattedAddress": (
                    slot.location.formatted_address if slot.location else None
                ),
            }
        ...
        if (
            ...
            and obj.contributors.filter(user=user, status="accepted")
            .instance_of(DateParticipant)
            .count()
        ):
...
class DateActivitySerializer(DateActivitySlotInfoMixin, TimeBasedBaseSerializer):
    ...
    my_contributor = SerializerMethodResourceRelatedField(
        model=DateParticipant, read_only=True, source="get_my_contributor"
    )
    ...
    first_slot = SerializerMethodResourceRelatedField(
        model=DateActivitySlot, read_only=True, source="get_first_slot"
    )
    ...
    def get_first_slot(self, instance):
        return (
            instance.slots.filter(start__gte=now())
    ...
    def get_contributors(self, instance):
        ...
        return [
            ...
            if (
                isinstance(contributor, DateParticipant)
                and (
                    contributor.status
                    in [
                    ...
                    or user
                    in (instance.owner, instance.initiative.owner, contributor.user)
                )
            )
        ]
    ...
    def get_unreviewed_contributors(self, instance):
        ...
        unreviewed_participants = instance.contributors.instance_of(
            DateParticipant
        ).filter(status=ParticipantStateMachine.new.value)

        if (
            user not in (instance.owner, instance.initiative.owner)
            and user not in instance.activity_managers.all()
        ):
    ...
    def get_my_contributor(self, instance):
        ...
        if user.is_authenticated:
            return (
                instance.contributors.filter(user=user)
                .instance_of(DateParticipant)
                .first()
            )
...
class DateActivityListSerializer(
    DateActivitySlotInfoMixin, TimeBasedActivityListSerializer
):
    date_info = serializers.SerializerMethodField()
    location_info = serializers.SerializerMethodField()
    ...
    class JSONAPIMeta(TimeBasedActivityListSerializer.JSONAPIMeta):
        resource_name = "activities/time-based/dates"
        included_resources = (
            TimeBasedActivityListSerializer.JSONAPIMeta.included_resources + ["slots"]
        )
...
class ParticipantListSerializer(BaseContributorSerializer):
    activity = PolymorphicResourceRelatedField(
        TimeBasedActivitySerializer, queryset=TimeBasedActivity.objects.all()
    )
...
class DateParticipantListSerializer(ParticipantListSerializer):
    slots = ResourceRelatedField(source="slot_participants", many=True, read_only=True)
    ...
    class JSONAPIMeta(ParticipantListSerializer.JSONAPIMeta):
        ...
        included_resources = (
            ParticipantListSerializer.JSONAPIMeta.included_resources
            + ["slots", "slots.slot"]
        )
...
class ParticipantSerializer(BaseContributorSerializer):
    ...
    document = PrivateDocumentField(
        required=False, allow_null=True, permissions=[ParticipantDocumentPermission]
    )
    ...
    class Meta(BaseContributorSerializer.Meta):
        ...
        validators = [
            UniqueTogetherValidator(
                queryset=DateParticipant.objects.all(), fields=("activity", "user")
            )
        ]
...
class DateParticipantSerializer(ParticipantSerializer):
    ...
    class Meta(ParticipantSerializer.Meta):
        model = DateParticipant
        meta_fields = ParticipantSerializer.Meta.meta_fields + (
            "permissions",
            "slot_count",
        )
