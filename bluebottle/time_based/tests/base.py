...
class TimeBasedActivityDetailAPITestCase:
    ...
    def setUp(self):
        self.model = self.factory.create(
            initiative=InitiativeFactory.create(status="approved"), **self.defaults
        )
    ...
    def test_export_url(self):
        ...
        self.assertTrue(
            "url"
            in self.response.json()["data"]["attributes"]["participants-export-url"]
        )
    ...
    def test_put_initiative_owner(self):
        new_description = "Test description"
        self.perform_update(
            {"description": new_description}, user=self.model.initiative.owner
        )
...
class TimeBasedRegistrationRelatedAPIListTestCase:
    ...
    def test_get(self):
        ...
        self.assertTrue(
            all(
                participant["meta"]["current-status"]["value"]
    ...
    def test_get_user_hide_first_name(self):
        ...
        for member in self.get_included("user"):
            if member["id"] == str(self.activity.owner.pk):
                self.assertIsNotNone(member["attributes"]["last-name"])
                self.assertEqual(
                    member["attributes"]["full-name"], self.activity.owner.full_name
                )
            else:
                self.assertIsNone(member["attributes"]["last-name"])
                self.assertEqual(
                    member["attributes"]["full-name"],
...
class TimeBasedRegistrationDetailAPITestCase:
    ...
    def test_update_user(self):
        ...
        self.perform_update(
            {"user": BlueBottleUserFactory.create()}, user=self.model.user
        )
        self.assertStatus(status.HTTP_200_OK)
