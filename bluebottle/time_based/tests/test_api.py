...
class TimeBasedListAPIViewTestCase:
    def setUp(self):
        ...
        self.data = {
            "data": {
                ...
                "relationships": {
                    "initiative": {
                        "data": {"type": "initiatives", "id": self.initiative.id},
                    },
                },
            }
        }
    ...
    def test_create_complete(self):
        ...
        self.assertEqual(
            self.response_data["attributes"]["title"],
            self.data["data"]["attributes"]["title"],
        )
        self.assertEqual(self.response_data["meta"]["permissions"]["GET"], True)

        self.assertEqual(self.response_data["meta"]["permissions"]["PUT"], True)

        self.assertEqual(self.response_data["meta"]["permissions"]["PATCH"], True)

    def test_create_duplicate_title(self):
        DateActivityFactory.create(title=self.data["data"]["attributes"]["title"])
    ...
    def test_create_as_activity_manager(self):
        ...
        response = self.client.post(
            self.url, json.dumps(self.data), user=activity_manager
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class DateListAPIViewTestCase(TimeBasedListAPIViewTestCase, BluebottleTestCase):
    ...
    def setUp(self):
        ...
        self.slot_data = {
            "data": {
                ...
                "relationships": {
                    "activity": {
                        "data": {"type": "activities/time-based/dates", "id": 0},
                    },
                },
            }
        }
    ...
    def test_create_complete(self):
        ...
        self.assertEqual(
            {
                transition["name"]
                for transition in self.response_data["meta"]["transitions"]
            },
            {"delete"},
        )
    ...
    def test_add_slots_by_owner(self):
        ...
        response = self.client.post(
            self.slot_url, json.dumps(self.slot_data), user=self.user
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_add_slots_by_other(self):
        ...
        response = self.client.post(
            self.slot_url, json.dumps(self.slot_data), user=other
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class TimeBasedDetailAPIViewTestCase:
            ...
            "data": {
                ...
                "relationships": {
                    "initiative": {
                        "data": {
                            "type": "initiatives",
                            "id": self.activity.initiative.id,
                        },
                    },
                },
            }
    ...
    def assertTransitionInData(self, transition, data):
        self.assertIn(
            transition, [trans["name"] for trans in data["meta"]["transitions"]]
        )
    ...
    def test_get_owner(self):
        ...
        self.assertEqual(data["meta"]["permissions"]["GET"], True)

        self.assertEqual(data["meta"]["permissions"]["PUT"], True)
    ...
    def test_export_with_segments(self):
        ...
        workshop = SegmentFactory.create(segment_type=department, name="Workshop")
        metal = SegmentFactory.create(segment_type=music, name="Metal")
        classical = SegmentFactory.create(segment_type=music, name="Classical")
        user = BlueBottleUserFactory.create()
        user.segments.add(workshop)
        user.segments.add(metal)
        user.segments.add(classical)
        participant = self.participant_factory.create(
            activity=self.activity, user=user, status="accepted"
        )
        for slot in self.activity.slots.all():
            SlotParticipantFactory.create(
                slot=slot, participant=participant, status="registered"
            )
    ...
    def test_get_contributors(self):
        ...
        self.assertEqual(self.response_data["meta"]["contributor-count"], 4)
        ...
        self.assertEqual(len(self.response_data), 5)
    ...
    def test_get_contributors_anonymous(self):
        ...
        self.assertEqual(len(self.response_data), 4)
    ...
    def test_get_non_anonymous(self):
        ...
        self.assertEqual(data["meta"]["permissions"]["PUT"], False)

        self.assertEqual(data["meta"]["permissions"]["PATCH"], False)
    ...
    def test_update_owner(self):
        response = self.client.put(
            self.url, json.dumps(self.data), user=self.activity.owner
        )
    ...
    def test_update_manager(self):
        response = self.client.put(
            ...
            user=self.activity.initiative.activity_managers.first(),
        )
...
class DateDetailAPIViewTestCase(TimeBasedDetailAPIViewTestCase, BluebottleTestCase):
    ...
    def test_matching_location_place_too_far(self):
        ...
        user = BlueBottleUserFactory.create()
        user.place = PlaceFactory.create(position=Point(x=4.9848386, y=52.3929661))
        user.save()
    ...
    def test_matching_location_location_too_far(self):
        ...
        user = BlueBottleUserFactory.create(
            location=LocationFactory.create(position=Point(x=4.9848386, y=52.3929661))
        )
...
class TimeBasedTransitionAPIViewTestCase:
    ...
    def test_delete_by_owner(self):
        ...
        self.assertEqual(
            data["included"][0]["type"], "activities/time-based/{}s".format(self.type)
        )
...
class DateTransitionAPIViewTestCase(
    TimeBasedTransitionAPIViewTestCase, BluebottleTestCase
):
    type = "date"
    factory = DateActivityFactory
    participant_factory = DateParticipantFactory
...
class DateActivitySlotListAPITestCase(BluebottleTestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(
            response.json()["meta"]["pagination"]["count"],
            len(self.activity.slots.all()),
        )
        self.assertEqual(
            response.json()["meta"]["total"], len(self.activity.slots.all())
        )
    ...
    def test_get_filtered_start(self):
        DateActivitySlotFactory.create(
            start=now() + timedelta(days=2), activity=self.activity
        )
        DateActivitySlotFactory.create(
            start=now() + timedelta(days=4), activity=self.activity
        )
        latest = DateActivitySlotFactory.create(
            start=now() + timedelta(days=6), activity=self.activity
        )
...
class ParticipantListViewTestCase():
    ...
    def test_create_with_document(self):
        ...
        data = response.json()['data']
        self.assertEqual(
            data["relationships"]["document"]["data"]["id"], document_data["data"]["id"]
        )
        private_doc = self.included_by_type(response, "private-documents")[0]
        self.assertTrue(
            private_doc["attributes"]["link"].startswith(
                "{}?signature=".format(
                    reverse(self.document_url_name, args=(data["id"],))
                )
            )
        )
...
class DateParticipantListAPIViewTestCase(
    ParticipantListViewTestCase, BluebottleTestCase
):
    type = "date"
    factory = DateActivityFactory
    participant_factory = DateParticipantFactory
...
class ParticipantDetailViewTestCase:
    def setUp(self):
        ...
        self.participant = self.participant_factory(
            activity=self.activity, motivation="My motivation"
        )
    ...
    def test_get_user(self):
        ...
        self.assertEqual(data["attributes"]["motivation"], self.participant.motivation)

        self.assertEqual(
            data["relationships"]["user"]["data"]["id"], str(self.participant.user.pk)
        )
        ...
        self.assertEqual(data["meta"]["permissions"]["PATCH"], True)
    ...
    def test_patch_document(self):
        ...
        self.data["data"]["relationships"] = {
            "document": {
                "data": {"type": "private-documents", "id": document_data["data"]["id"]}
            }
        }

        response = self.client.patch(
            self.url, json.dumps(self.data), user=self.participant.user
        )
...
class DateParticipantDetailAPIViewTestCase(
    ParticipantDetailViewTestCase, BluebottleTestCase
):
    type = "date"
    factory = DateActivityFactory
    participant_factory = DateParticipantFactory
...
class ParticipantTransitionAPIViewTestCase:
    ...
    def test_withdraw_by_user(self):
        ...
        participant = [
            ...
            if include["type"] == "{}s".format(self.participant_type)
        ]
...
class DateParticipantTransitionAPIViewTestCase(
    ParticipantTransitionAPIViewTestCase, BluebottleTestCase
):
    type = "date"
...
class ReviewParticipantTransitionAPIViewTestCase:
    def setUp(self):
        ...
        self.participant = self.participant_factory.create(
            activity=self.activity, user=self.user, as_user=self.user
        )
    ...
    def test_withdraw_by_other_user(self):
        ...
        response = self.client.post(
            self.url, json.dumps(self.data), user=self.another_user
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class RelatedParticipantsAPIViewTestCase:
        ...
        for i in range(10):
            self.participants.append(
                self.participant_factory.create(
                    activity=self.activity, document=PrivateDocumentFactory.create()
                )
            )
    ...
    def assertTotal(self, total):
        return self.assertEqual(
            self.response.json()["meta"]["pagination"]["count"], total
        )
    ...
    def test_get_owner_disable_last_name(self):
        MemberPlatformSettings.objects.update_or_create(
            display_member_names="first_name"
        )
        self.response = self.client.get(self.url, user=self.activity.owner)
    ...
    def test_get_closed_site(self):
        ...
        group.permissions.remove(
            Permission.objects.get(codename="api_read_dateparticipant")
        )
        group.permissions.remove(
            Permission.objects.get(codename="api_read_periodparticipant")
        )
...
class RelatedDateParticipantAPIViewTestCase(
    RelatedParticipantsAPIViewTestCase, BluebottleTestCase
):
    type = "date"
...
class SlotParticipantListAPIViewTestCase(BluebottleTestCase):
    ...
    def test_create_participant_user(self):
        ...
        self.assertEqual(data["relationships"]["slot"]["data"]["id"], str(self.slot.pk))
...
class SlotParticipantDetailAPIViewTestCase(BluebottleTestCase):
    def setUp(self):
        ...
        self.activity = DateActivityFactory.create(review=False, owner=self.owner)
        self.slot = DateActivitySlotFactory.create(activity=self.activity)
        self.participant1 = DateParticipantFactory.create(
            user=self.supporter1, activity=self.activity
        )
        ...
        self.participant2 = DateParticipantFactory.create(
            user=self.supporter2, activity=self.activity
        )
        SlotParticipantFactory.create(slot=self.slot, participant=self.participant2)
        self.participant2.states.withdraw(save=True)

        p1_sl1 = SlotParticipant.objects.get(
            slot=self.slot, participant=self.participant1
        )
        p2_sl1 = SlotParticipant.objects.get(
            slot=self.slot, participant=self.participant2
        )
...
class SlotParticipantTransitionAPIViewTestCase(BluebottleTestCase):
    ...
    def test_withdraw_by_user(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(
            data["included"][2]["type"], "contributors/time-based/slot-participants"
        )
...
class DateTimeContributionAPIViewTestCase(
    TimeContributionDetailAPIViewTestCase, BluebottleTestCase
):
    factory = DateActivityFactory
    participant_factory = DateParticipantFactory
...
class SlotIcalTestCase(BluebottleTestCase):
    ...
    def test_get(self):
        ...
        for ical_event in calendar.walk("vevent"):
            self.assertAlmostEqual(
                ical_event["dtstart"].dt, self.slot.start, delta=timedelta(seconds=10)
            )
            self.assertAlmostEqual(
                ical_event["dtend"].dt,
    ...
    def test_get_wrong_signature(self):
        response = self.client.get(
            "{}?signature=ewiorjewoijical_url".format(self.unsigned_url)
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class DateIcalTestCase(BluebottleTestCase):
    def setUp(self):
        ...
        self.activity = DateActivityFactory.create(
            title="Pollute Katwijk Beach", slots=[]
        )
    ...
    def test_get_applied_to_all(self):
        ...
        for index, ical_event in enumerate(calendar.walk("vevent")):
            ...
            self.assertAlmostEqual(
                ical_event["dtstart"].dt, slot.start, delta=timedelta(seconds=10)
            )
            self.assertAlmostEqual(
                ical_event["dtend"].dt,
            ...
            self.assertEqual(
                str(ical_event["description"]),
                "{}\nJoin: {}".format(self.activity.details, slot.online_meeting_url),
            )
            self.assertEqual(ical_event["url"], self.activity.get_absolute_url())
            self.assertEqual(
                ical_event["organizer"], "MAILTO:{}".format(self.activity.owner.email)
...
class SlotRelatedParticipantListTestCase(APITestCase):
    def setUp(self):
        ...
        self.participants = DateParticipantFactory.create_batch(
