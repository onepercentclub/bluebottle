...
def slot_is_started(effect):
    ...
    """
    return (
        effect.instance.is_complete
        and effect.instance.start
        and effect.instance.start < now()
    )
...
def slot_is_finished(effect):
    ...
    return (
        effect.instance.is_complete
        and effect.instance.end
        and effect.instance.end < now()
    )
...
def slot_is_full(effect):
    ...
    participant_count = effect.instance.slot_participants.filter(
        participant__status="accepted", status__in=["registered", "succeeded"]
    ).count()
    ...
    triggers = [
        TransitionTrigger(
            ...
            effects=[
                ...
                TransitionEffect(
                    ActivitySlotStateMachine.finish, conditions=[slot_is_finished]
                ),
            ],
        ),
        TransitionTrigger(
        ...
        ModelChangedTrigger(
            ...
            effects=[
                TransitionEffect(
                    ActivitySlotStateMachine.lock, conditions=[slot_is_full]
                ),
                TransitionEffect(
                    ActivitySlotStateMachine.unlock, conditions=[slot_is_not_full]
                ),
            ],
        ),
        TransitionTrigger(
...
def all_slots_cancelled(effect):
    ...
    """
    return (
        effect.instance.activity.slots.exclude(status__in=["cancelled", "deleted"])
        .exclude(
...
def activity_will_be_full(effect):
    ...
    if activity.team_activity == "teams":
        accepted_teams = activity.teams.filter(
    ...
    return (
        activity.capacity
        and activity.capacity == len(activity.accepted_participants) + 1
    )
...
def activity_will_not_be_full(effect):
    ...
    return not activity.capacity or activity.capacity >= len(
        activity.accepted_participants
    )
...
def activity_is_finished(effect):
    ...
    return (
        last_slot
        and last_slot.start
        and last_slot.duration
        and last_slot.start + last_slot.duration < now()
    )
    ...
    triggers = [
        TransitionTrigger(
            ...
            effects=[
                ...
                NotificationEffect(
                    ManagerSlotParticipantRegisteredNotification,
                    conditions=[applicant_is_accepted, is_participant],
                ),
                NotificationEffect(
                    ParticipantSlotParticipantRegisteredNotification,
                    conditions=[applicant_is_accepted, is_participant],
