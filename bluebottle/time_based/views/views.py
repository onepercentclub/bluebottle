...
class RelatedSlotParticipantListView(
...
class DateSlotListView(JsonApiViewMixin, ListCreateAPIView):
    ...
    def filter_queryset(self, queryset):
        ...
        try:
            activity_id = self.kwargs.get("pk", None) or self.request.GET.get(
                "activity"
            )
            ...
            raise ValidationError(
                "Invalid parameter: activity ({})".format(activity_id)
            )
        ...
        try:
            ...
            raise ValidationError(
                "Invalid parameter: contributor ({})".format(contributor_id)
            )
        except KeyError:
            pass
...
class SlotRelatedParticipantList(JsonApiViewMixin, ListAPIView):
    ...
    def get_serializer_context(self, **kwargs):
        ...
        if self.request.user:
            ...
            if (
                ...
                or self.request.user in activity.initiative.activity_managers.all()
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        activity = DateActivity.objects.get(slots=self.kwargs["slot_id"])
        queryset = (
            super().get_queryset(*args, **kwargs).filter(slot_id=self.kwargs["slot_id"])
        )
        ...
        if user.is_anonymous:
            ...
            user != activity.owner
            and user != activity.initiative.owner
            and user not in activity.initiative.activity_managers.all()
...
class SlotParticipantListView(JsonApiViewMixin, CreateAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        return (
            ...
            .filter(participant__status__in=["new", "accepted"])
        )
...
class DateActivityIcalView(PrivateFileView):
    ...
    def get(self, *args, **kwargs):
        ...
        if kwargs.get("user_id"):
            slots = slots.filter(
                slot_participants__participant__user__id=kwargs["user_id"]
            )
...
class DateParticipantExportView(ExportView):
    ...
    def get_fields(self):
        ...
        segments = tuple(
            (f"segment.{segment.pk}", segment.name)
    ...
    def write_data(self, workbook):
        ...
        for slot in activity.active_slots.filter(start__gt=now()).order_by("start"):
            title = (
                f"{slot.start.strftime('%d-%m-%y %H:%M')} {slot.id} {slot.title or ''}"
            )
    ...
    def get_instances(self):
        return (
            self.get_object()
            .contributors.instance_of(DateParticipant)
